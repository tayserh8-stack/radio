...
import { getAllPrompts, updatePrompt, resetPrompts } from '../../services/coupletPromptService';
...
const CoupletPromptManagement = () => {
  ...
  const isAuthorized = user && (user.role === 'admin' || (() => {
    ...
    return dept === 'news' || dept === 'الأخبار' || dept.includes('news') || dept.includes('إعلام');
  })());
  ...
  const handleReset = async () => {
    if (!isAuthorized) {
      setError('غير مصرح لك بإعادة التعيين');
      return;
    }
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto" dir="rtl">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">إدارة برومتات التحرير الثانية</h1>
          <p className="text-gray-500 text-sm mt-1">إدارة التعليمات (البرومتات) لتحرير الفيديو جراف (التنسيق الرباعي الثنائي)</p>
        </div>
        <button
          onClick={handleReset}
          disabled={loading || !isAuthorized}
          className={`px-4 py-2 rounded-lg transition-colors text-sm font-medium border flex items-center gap-2 ${
            isAuthorized
      ...
      {!isAuthorized && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-700 flex items-center gap-2">
          ...
          <span>لديك صلاحية عرض البرومتات فقط. لا يمكنك تعديل أو حفظ التغييرات.</span>
        </div>
      )}
      ...
      {error && (
        <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg text-sm text-red-600 flex items-center justify-between">
          <span>{error}</span>
          <button onClick={loadPrompts} className="px-3 py-1 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition-colors text-xs font-medium">
            إعادة المحاولة
          </button>
        </div>
      )}
      ...
      {filteredStages.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredStages.map((stage) => {
            ...
            return (
              <div key={stage.id} className={`bg-white rounded-xl shadow-sm border overflow-hidden transition-all duration-200 ${
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {!isEditing && (
                      <div className="flex items-center gap-1 flex-shrink-0">
                        {promptData?.prompt && (
                          <button onClick={() => handleCopy(promptData.prompt, stage.id)} className="p-2 text-gray-400 hover:text-primary hover:bg-primary/5 rounded-lg transition-colors" title="نسخ البرومت">
                        ...
                        <button onClick={() => startEdit(promptData || { stage: stage.id, prompt: '', name: stage.name, description: stage.desc })}
                          disabled={!isAuthorized}
                          className={`px-3 py-1.5 text-sm rounded-lg transition-colors font-medium ${
                            isAuthorized ? 'text-primary hover:bg-primary/5' : 'text-gray-300 cursor-not-allowed'
                          }`}>
                          تعديل
                        </button>
                      </div>
                  ...
                  {isEditing ? (
                    <div className="space-y-3 mt-4 mr-12">
                      ...
                      <div>
                        ...
                        <textarea value={editForm.prompt}
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => handleSave(stage.id)}
                        ...
                        <button onClick={cancelEdit} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors text-sm font-medium">
                          إلغاء
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="mt-3 mr-12">
                      <div className="p-3 bg-gray-50 rounded-lg border border-gray-100 relative group">
                        {promptData?.prompt ? (
                          <pre className="text-sm text-gray-700 whitespace-pre-wrap font-sans leading-relaxed">{promptData.prompt}</pre>
                        ) : (
...
export default CoupletPromptManagement;
